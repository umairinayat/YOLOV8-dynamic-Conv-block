...
def arg_parser():
    ...
    parser.add_argument("--data", type=str, default="space", help="Path to dataset")
...
def main(opt):
    ...
    if opt.load_coco_weights:
        ...
        if opt.data == "coco":
            model.load_state_dict(torch.load("yolov8n_coco.pt"), strict=True)
            # loads all coco weights but the heads
        else:
            model.load_state_dict(torch.load("yolov8n_space_nh.pt"), strict=False)
